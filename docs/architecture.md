...
- [`auto-push-bridge.ts`](../extensions/loom/auto-push-bridge.ts) / [`auto-push-manager.ts`](../extensions/loom/auto-push-manager.ts) -- experimental notebook → Galaxy auto-push (gated by `LOOM_AUTO_PUSH=1` / `experiments.autoPush`, off by default). On a genuine edit to a bound + connected notebook, debounce-pushes to the Galaxy page so the embedded view tracks local edits; dedups on the housekeeping-stripped body so the push's own self-write doesn't loop. Local-wins, so it clobbers server-side edits -- the reason it stays opt-in.
