...
- [`index.ts`](../extensions/loom/index.ts) -- entry wiring: registers tools, slash commands, lifecycle hooks, UI bridge, context injection. Registers the exec-guard first so its decision is the authoritative boundary.
- [`exec-guard/`](../extensions/loom/exec-guard/) -- the local-execution safety gate. Subscribes to Pi's `tool_call` event and returns `{ block }` for `bash`/`write`/`edit`/`read`. A pure `policy.ts` (`allow`/`ask`/`deny`) consumes four classifiers (`model-tier`, `sensitive-read`, `bash-risk`, realpath `path-jail`); `gate.ts` performs the `ask` via `ctx.ui` (fails closed when `ctx.hasUI` is false) and audits every decision to `activity.jsonl`. Shell-neutral: all policy lives in the brain, prompts render through the `ctx.ui` abstraction. A human-only `dangerouslyBypassPermissions` mode (config / `--dangerously-bypass-permissions` / Orbit toggle) turns it into a pass-through. See [README "Local execution safety"](../README.md#local-execution-safety). Writes via the `write`/`edit` tools are confined to the analysis directory by default (out-of-dir or credential-shaped paths prompt); this is on by default, every platform.
- [`sandbox/`](../extensions/loom/sandbox/) -- the opt-in OS bash sandbox, layered UNDER the exec-guard. When enabled (`--sandbox` / `LOOM_SANDBOX` / `guardian.sandbox`, macOS + Linux/WSL2) it wraps `bash` via `@anthropic-ai/sandbox-runtime` so an allowed command's writes are confined to the workspace and its network is limited. Off by default because enabling it necessarily restricts bash network (ASRT can't confine writes alone). The gate still decides `allow`/`ask`/`deny`; the sandbox only changes how an already-allowed command runs. The default-on confinement of the model's `write`/`edit` file writes (above) lives in the gate itself, independent of this sandbox.
...
- [`skills-discovery.ts`](../extensions/loom/skills-discovery.ts) -- the skill catalog: frontmatter parsing, `surfaces: [loom]` tag-or-all selection, the GitHub tree-walk and its 1h cache. `resolveCatalogEntries` prefers the bundled catalog and falls back to the cached walk; background refresh runs for live repos only.
- [`vendor-skills.ts`](../extensions/loom/vendor-skills.ts) + [`vendor/skills/`](../extensions/loom/vendor/skills/) -- skill content that ships inside the package, vendored from `galaxyproject/agentic-plugins` at the commit pinned in [`scripts/skills.manifest.json`](../scripts/skills.manifest.json) by [`scripts/sync-skills.mjs`](../scripts/sync-skills.mjs). Two plugins: the galaxy-skills mirror, which backs the `galaxy-skills` repo and generates `_catalog.json` for the router, and the Foundry casts, which are reachable only through `skills_fetch({ repo: "foundry" })` and never enter the router. `npm run check:skills` is the offline drift gate; `shared/skills-pin.js` carries the pin to the shells.
- [`invocation-failure-hint.ts`](../extensions/loom/invocation-failure-hint.ts) -- appends a triage nudge to a `galaxy_invocation_check_*` result that reports a fresh `failed` transition, naming the two bundled reference notes by path. Sibling of `confusables-hint.ts`.
...
- [`loom-config.js`](../shared/loom-config.js) -- canonical `~/.loom/config.json` load/save. Atomic write (`tmp` + `rename`), `0600` permissions, lazy-seeds `galaxy-skills` if no skill repos are configured. `isBundledSkillRepo` decides whether a configured repo reads from the package or is fetched, and both the brain and Preferences use it so they cannot disagree. Skill-repo URLs are restricted to `github.com/galaxyproject/*` (see `ALLOWED_SKILLS_PREFIX`); the agent treats fetched SKILL.md content as authoritative, so an arbitrary repo would be a prompt-injection vector.
