...
analyses in Galaxy.`,
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      // The parameter form is useful in two modes:
      //   1. plan committed + has tool/workflow steps → form submit routes
      //      to `/execute {savedParameters:...}`.
      //   2. plan still being drafted (no state.currentPlan yet, or plan
      //      exists without executable steps) → form submit drops the
      //      values into the chat input so the researcher can hand them
      //      to the agent for the next drafting turn.
      // So there's no hard guard: the tool always renders the form. The
      // renderer decides the submit-button destination from the payload.
      const plan = getCurrentPlan();
